...
Public Class MixerSaturationCommand
    ...
    Public Sub New(ByVal channel As Integer, ByVal layer As Integer, ByVal saturation As Single)
        MyBase.New("MIXER SATURATION", "Changes the saturation of the specified layer. The value is a float between 0 and 1")
        InitParameter()
        setChannel(channel)
    ...
    Public Sub New(ByVal channel As Integer, ByVal layer As Integer, ByVal saturation As Single, ByVal duration As Integer, Optional ByVal tween As CasparCGUtil.Tweens = CasparCGUtil.Tweens.linear)
        MyBase.New("MIXER SATURATION", "Changes the saturation of the specified layer. The value is a float between 0 and 1")
        InitParameter()
        setChannel(channel)
        If layer > -1 Then setLayer(layer)
    ...
    Private Sub InitParameter()
        ...
        addCommandParameter(New CommandParameter(Of Single)("saturation", "The saturation value of the layer", 0.0, True))
    ...
    Public Overrides Function getCommandString() As String
        checkParameter()
        Dim cmd As String = "MIXER " & getDestination() & " SATURATION"

        If getCommandParameter("saturation").isSet Then
            cmd = cmd & " " & getSaturation.ToString(CultureInfo.GetCultureInfo("en-US"))
    ...
    Public Function getSaturation() As Single
        Dim param As CommandParameter(Of Single) = DirectCast(getCommandParameter("saturation"), CommandParameter(Of Single))
        If param IsNot Nothing And param.isSet Then
            Return param.getValue
        Else
            Return param.getDefault
