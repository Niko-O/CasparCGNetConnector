...
Public Class MixerBrightnessCommand
    Inherits AbstractCommand

    Public Sub New()
        MyBase.New("MIXER BRIGHTNESS", "Changes the brightness of the specified layer. The value is a float between 0 and 1")
        InitParameter()
    End Sub

    Public Sub New(ByVal channel As Integer, ByVal layer As Integer, ByVal brightness As Single)
        MyBase.New("MIXER BRIGHTNESS", "Changes the brightness of the specified layer. The value is a float between 0 and 1")
        InitParameter()
        setChannel(channel)
    ...
    Public Sub New(ByVal channel As Integer, ByVal layer As Integer, ByVal brightness As Single, ByVal duration As Integer, Optional ByVal tween As CasparCGUtil.Tweens = CasparCGUtil.Tweens.linear)
        MyBase.New("MIXER BRIGHTNESS", "Changes the brightness of the specified layer. The value is a float between 0 and 1")
        InitParameter()
        setChannel(channel)
        If layer > -1 Then setLayer(layer)
    ...
    Public Sub setBrightness(ByVal brightness As Single)
        ...
        '    DirectCast(getCommandParameter("brightness"), CommandParameter(Of Single)).setValue(1)
        'Else
        DirectCast(getCommandParameter("brightness"), CommandParameter(Of Single)).setValue(brightness)
        'End If
    End Sub

    Public Function getBrightness() As Single
        Dim param As CommandParameter(Of Single) = DirectCast(getCommandParameter("brightness"), CommandParameter(Of Single))
        If param IsNot Nothing And param.isSet Then
            Return param.getValue
        Else
            Return param.getDefault
